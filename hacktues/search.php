...
$results = $mysqli->query("SELECT * FROM prodexu.products
            WHERE (`product_name` LIKE '%".$query."%') OR (`product_desc` LIKE '%".$query."%')") or die(mysql_error());
if(mysqli_num_rows($results) > 0){ // if one or more rows are returned do following

if($results){ 
$products_item = '<ul class="products">';
//fetch results set as object and output HTML
echo "<h1 align = center style=margin-bottom:30px> Резултати от търсенето </h1>";
...
echo $products_item;
	...
	echo "<h1 style=margin-bottom:30px align=center >Няма резултати</h1>";
	echo '<div center=align><img src="img/5dbc9266a3fe300d521c68b25c4c33f1.png" ></div>';
